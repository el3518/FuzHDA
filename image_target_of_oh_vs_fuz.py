...
###########
import src.methods.oh.source_fuz as SOURCE_fuz
import src.methods.oh.bb_target as bb_TARGET
import src.methods.oh.ht_lcfd_fuz as ht_LCFD_fuz
############
...
if __name__ == "__main__":
    ...
    if cfg.SETTING.DATASET == 'office-home':
        ...
        if cfg.DA == 'oda':
            cfg.class_num = 25
            cfg.src_classes = [i for i in range(25)]
            cfg.tar_classes = [i for i in range(65)]
    ...
    elif cfg.MODEL.METHOD == "source_fuz":
        print("training source model")
        acc = SOURCE_fuz.train_source(cfg)
    elif cfg.MODEL.METHOD == "bb_target":
        print("training source model")
        acc = bb_TARGET.train_bb_target(cfg) 
    elif cfg.MODEL.METHOD == "ht_lcfd_fuz":
        print("training source model")
        acc = ht_LCFD_fuz.train_target(cfg)  
